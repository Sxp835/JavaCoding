...
public class Library {
  ...
  public String lookup(long isbn) {
    ...
    for(i=0;i< library.size(); ++i) {
    	book1 = (LibraryBook)library.get(i);
    	isbnLooking = book1.getIsbn();
    	if(isbnLooking == isbn) {
    		return book1.getHolder();
    	}
  ...
  public boolean checkout(long isbn, String holder, int month, int day, int year) {
    // *FILL IN -- do not return false unless appropriate
	  ...
	  for(i=0;i< library.size(); ++i) {
	    	...
	    	isbnLooking = book1.getIsbn();
	    	if(isbnLooking == isbn) {
	    		if(book1.getHolder() != null) {
	    			return false;
	    		}
	    		else {
	    			returndate = new GregorianCalendar(year, month, day);
	    			book1.checkout(holder, returndate);
	    			return true;
	    		}
	    	}
	  }
  ...
  public ArrayList<LibraryBook> lookup(String holder) {
	  ...
	  int i;
	  LibraryBook book2;
	  String holderLooking;
	  
	  
	  for(i=0;i<library.size();++i) {
		  book2 = (LibraryBook) library.get(i);
		  holderLooking = (String) book2.getHolder();
		  if(holderLooking!= null) {
			  if(holderLooking.equals(holder)) {
				  holderList.add(book2);
  ...
  public boolean checkin(String holder) {
	  ...
	  for(i=0;i< library.size(); ++i) {
	    	book1 = (LibraryBook)library.get(i);
	    	holderLooking = book1.getHolder();
	    	if(holderLooking == holder) {
	    			book1.checkin();
	    			library.add(i,book1);
	    			return true;
	    	}
